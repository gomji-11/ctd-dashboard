...
function render(products) {
  ...
  replaceChart("conversionChart", {
    ...
    options: { responsive: true, maintainAspectRatio: false, cutout: "66%", plugins: { centerText: { stats: { total: ownProducts.length, converted: ownConvertedCount, rate: ownProducts.length ? Math.round(ownConvertedCount / ownProducts.length * 100) : 0 } }, legend: commonLegend, tooltip: { callbacks: { label: item => ` ${item.label}: ${item.raw}개` } } } },
